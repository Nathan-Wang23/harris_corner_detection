...
def compute_image_gradients(image_bw: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    padding = int((k - 1)/2)
    padding2 = int((j - 1)/2)
    Ix_padded = np.pad(Ix, pad_width=((padding, padding),(padding2, padding2)), constant_values = 0)
    Iy_padded = np.pad(Ix, pad_width=((padding, padding),(padding2, padding2)), constant_values = 0)

    for row in range(0, m):
        for col in range(0, n):
          new_value_X = np.multiply(Ix_padded[row : row+k, col:col+j], SOBEL_X_KERNEL)
          Ix[row, col] =  new_value_X.sum()

          new_value_Y = np.multiply(Iy_padded[row : row+k, col:col+j], SOBEL_Y_KERNEL)
          Iy[row, col] =  new_value_Y.sum()
...
def compute_harris_response_map(
    ...
    # det = g(Ixx) g(Iyy) - (g(Ixy))^2
    det_M = (sxx * syy) - (sxy**2)

    # Trace = g(Ixx) + g(Iyy)
    trace = sxx + syy
    trace_M_2 = trace * trace



    R = det_M - alpha * trace_M_2
...
def nms_maxpool_pytorch(
    ...
    ###########################################################################
    r = R.copy()
    median = torch.median(torch.Tensor(r)).item()
    ...
    maxpooled_R = maxpool.squeeze(0).squeeze(0)

    rr = rr.squeeze(0).squeeze(0)
    ...
    interests = np.argwhere(rr != 0)
    x = interests[:, 1]
    y = interests[:, 0]

    arg_sort = np.argsort(rr[y, x])
    arg_sort = arg_sort[::-1][:k]
    x = x[arg_sort]
    y = y[arg_sort]
    confidences = rr[y, x]
...
def get_harris_interest_points(
    ...
    score = compute_harris_response_map(image_bw)


    score = (score - np.min(score)) / (np.max(score) - np.min(score))
    x, y, c = nms_maxpool_pytorch(score, k, 7)
