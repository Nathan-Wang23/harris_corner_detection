...
def get_gradient_histogram_vec_from_patch(
    ...
    ###########################################################################
    wgh = []
    bins = (np.arange(9) - 4) * np.pi / 4
    bins[0:9] = bins[0:9] - 1e-6

    i = 0
    while i <= 12:
        j = 0
        while j <= 12:
            grid_mag = window_magnitudes[i:i+4, j:j+4]
            grid_orient = window_orientations[i:i+4, j:j+4]

            histo= np.histogram(grid_orient, bins=bins, range=(-np.pi, np.pi), weights=grid_mag)
            wgh.append(histo[0])
            j += 4
        i += 4
    wgh = np.array(wgh)
...
def get_feat_vec(
    ...
    window_magnitude = magnitudes[int(r) - (feature_width-1)//2: int(r) + feature_width//2 + 1, int(c) - (feature_width-1)//2 : int(c) + feature_width//2 + 1]
    window_orientation = orientations[int(r) - (feature_width-1)//2: int(r) + feature_width//2 + 1, int(c) - (feature_width-1)//2 : int(c) + feature_width//2 + 1]


    fv = get_gradient_histogram_vec_from_patch(window_magnitude, window_orientation)


    fv /= np.linalg.norm(fv, axis=0)
...
def get_SIFT_descriptors(
    ...
    for i in range(len(X)):
        fvs.append(get_feat_vec(X[i], Y[i], magnitudes, orientations, feature_width).reshape(128,))
    fvs = np.array(fvs)
...
def get_sift_features_vectorized(
    ...
    for i in range(len(X)):
        fvs.append(get_feat_vec(X[i], Y[i], magnitudes, orientations).reshape(128,))
    fvs = np.array(fvs)
